...
DODO Trade API 当前可提供 Ethereum、BSC、Polygon、Arbitrum One、HECO、OEC、Aurora、MoonRiver 八个网络下任意币互换的实时报价以及可直接使用的与合约交互ABI数据。开发者仅需按照接口规范，集成我们的API，即可快速实现DEX最重要的交易功能。DODO Trade API 背后集成了DODOV1、DODOV2、专业做市商、1inch API、0x API、ParaSwap API 以及DODO自建的聚合算法 等多源实时的报价信息，并返回当前多源中最优报价，以确保 DODO Trade API 始终具有较强的竞争力。
...
- **Polygon：** DODOV2、0x API、ParaSwap API、Wootrade做市商、DODO自建路由

- **Arbitrum One:** DODOV1, DODOV2, 1inch API、DODO自建路由

- **MoonRiver：** DODOV2，DODO自建路由
...
| chainId                     | integer      | (必须)  1 代表以太主网, 56 代表 BSC, 66 代表 OEC, 128 代表 Heco, 137 代表 Polygon, 42161 代表 Arbitrum One, 1285 代表 MoonRiver, 1313161554 代表 Aurora |
